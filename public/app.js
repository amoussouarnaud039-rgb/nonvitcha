...
function setupEventListeners() {
    ...
    if (privateInput) {
        privateInput.addEventListener('input', () => {
            if (activePrivateChatUserId && currentUser) {
                // Envoi d'événement "typing" aligné avec le serveur (toUserId)
                socket.emit('typing', { toUserId: activePrivateChatUserId });
    ...
    if (privateForm) {
        privateForm.addEventListener('submit', (e) => {
            ...
            // Envoi de message privé aligné avec toUserId
            ...
            // Arrêt immédiat de l'indicateur d'écriture à l'envoi du message
            socket.emit('stop_typing', { toUserId: activePrivateChatUserId });
...
socket.on('private_message', (data) => {
    const isOwn = currentUser && (data.fromUserId === currentUser.id);
...
socket.on('user_typing', (data) => {
    if (activePrivateChatUserId === data.fromUserId) {
